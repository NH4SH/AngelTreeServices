...
export function DeliveryForm({ customers, equipment, jobs, locations, materials }: { customers: BasicOption[]; equipment: BasicOption[]; jobs: BasicOption[]; locations: InventoryLocationRecord[]; materials: MaterialRecord[] }) {
  const [state, action, pending] = useActionState(recordCustomerDelivery, initialState);
  return <form action={action} className="crm-form material-form" encType="multipart/form-data"><IdempotencyInput /><FormMessage state={state} />
    <div className="form-grid-three"><label>Customer<select name="customer_id" required><option value="">Choose customer</option>{customers.map((customer) => <option key={customer.id} value={customer.id}>{customer.display_name}</option>)}</select></label><MaterialSelect materials={materials} /><JobSelect jobs={jobs} /></div>
