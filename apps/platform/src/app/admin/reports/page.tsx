...
function CustomerReport({ data }: { data: ReportData }) {
  const revenueByCustomer = new Map<string, number>(); data.invoices.filter((invoice) => invoice.status !== "void").forEach((invoice) => revenueByCustomer.set(contractingPartyKey(invoice), (revenueByCustomer.get(contractingPartyKey(invoice)) ?? 0) + invoice.total_cents));
  const rows = [...revenueByCustomer.entries()].map(([partyKey, value]) => ({ partyKey, label: contractingPartyName(data.invoices.find((invoice) => contractingPartyKey(invoice) === partyKey)), value })).sort((a, b) => b.value - a.value);
  const repeat = groupCount(data.jobs.filter(isCompletedJob), contractingPartyKey).filter((row) => row.value > 1).length;
...
function quoteRow(quote: ReportQuote): React.ReactNode[] { return [<Link href={`/admin/quotes/${quote.id}`} key="quote">{quote.quote_number || "Draft quote"}</Link>, contractingPartyName(quote), relation(quote.profiles)?.full_name || relation(quote.profiles)?.email || "Not assigned", title(quote.status), money(quote.total_cents), quote.sent_at ? formatDate(quote.sent_at) : `${ageDays(quote.created_at)} days old`]; }
function invoiceRow(invoice: ReportInvoice): React.ReactNode[] { return [<Link href={`/admin/invoices/${invoice.id}`} key="invoice">{invoice.invoice_number || "Draft invoice"}</Link>, contractingPartyName(invoice), invoice.due_at ? `${formatDate(invoice.due_at)}${daysOverdue(invoice) > 0 ? ` · ${daysOverdue(invoice)}d overdue` : ""}` : "Missing", money(invoice.total_cents), money(successfulPaymentTotal(invoice)), money(invoice.balance_due_cents)]; }
function contractingPartyKey(record: { customer_id: string | null; organization_id: string | null }) { return record.organization_id ? `organization:${record.organization_id}` : `customer:${record.customer_id ?? "unknown"}`; }
function contractingPartyName(record?: { customers: unknown; organizations: unknown } | null) { const organization = relation(record?.organizations as never) as { name?: string } | null; const customer = relation(record?.customers as never) as { display_name?: string } | null; return organization?.name ?? customer?.display_name ?? "Unknown"; }
