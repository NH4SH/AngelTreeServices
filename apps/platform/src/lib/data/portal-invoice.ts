...
export async function getInvoiceByPortalToken(rawToken: string): Promise<PortalInvoiceLookup> {
  ...
  const { data: invoice, error: invoiceError } = await supabase
    .from("invoices")
    .select(
      "*, jobs(id, status, service_type, requested_scope, service_locations(id, label, street, city, state, postal_code)), customers:customers!invoices_customer_id_fkey(id, display_name, phone, email), organizations(id, name, billing_phone, billing_email), invoice_line_items(*), payments(*)",
    )
