-- Source-aware estimate scheduling and reversible website-lead classification.
...
alter table public.schedule_events
  ...
  add column if not exists source_contact_id uuid references public.organization_contacts(id) on delete set null,
  add column if not exists source_service_type text,
  add column if not exists source_request_key uuid;

create unique index if not exists schedule_events_source_request_key_uidx
  on public.schedule_events(source_request_key)
  where source_request_key is not null;
create index if not exists schedule_events_source_customer_idx
  on public.schedule_events(source_customer_id)
  where source_customer_id is not null;
create index if not exists schedule_events_source_organization_idx
  on public.schedule_events(source_organization_id)
  where source_organization_id is not null;

alter table public.jobs
  add column if not exists lead_disposition text not null default 'active';

alter table public.jobs
  drop constraint if exists jobs_lead_disposition_check;
alter table public.jobs
  add constraint jobs_lead_disposition_check
  check (lead_disposition in ('active', 'spam', 'archived'));

update public.jobs
set lead_disposition = 'archived'
where website_submission_id is not null
  and archived_at is not null
  and lead_disposition = 'active';

create index if not exists jobs_website_lead_disposition_submitted_idx
  on public.jobs(lead_disposition, submitted_at desc)
  where website_submission_id is not null;

create or replace function public.prevent_inactive_lead_conversion()
returns trigger
language plpgsql
set search_path = ''
as $$
begin
  if old.website_submission_id is not null
    and old.lead_disposition <> 'active'
    and new.status is distinct from old.status
  then
    raise exception using
      errcode = '22023',
      message = 'Restore this lead before converting or scheduling it.';
  end if;
  return new;
end;
$$;

drop trigger if exists jobs_prevent_inactive_lead_conversion on public.jobs;
create trigger jobs_prevent_inactive_lead_conversion
  before update of status on public.jobs
  for each row execute function public.prevent_inactive_lead_conversion();
...
declare
  ...
  existing public.schedule_events%rowtype;
...
revoke all on function public.schedule_party_estimate(
...
grant execute on function public.schedule_party_estimate(
  uuid, uuid, uuid, uuid, uuid, text, text, text, text, text, text,
  text, text, text, text, text, text, text, timestamptz, timestamptz, uuid
) to authenticated;

comment on column public.jobs.lead_disposition is
  'Office classification for website lead queues. Active, spam, and archived are reversible.';
comment on column public.schedule_events.source_request_key is
  'Per-form idempotency key preventing repeat estimate scheduling submissions.';
