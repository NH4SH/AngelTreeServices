...
function exportRows(view: string, data: Awaited<ReturnType<typeof getReportData>>): Array<Array<string | number | null>> {
  if (view === "revenue") return [["Invoice", "Customer", "Status", "Invoice total cents", "Successful payments cents", "Balance due cents", "Created at", "Due at", "Paid at"], ...data.invoices.map((invoice) => [invoice.invoice_number, relation(invoice.customers)?.display_name ?? "Unknown", invoice.status, invoice.total_cents, successfulPaymentTotal(invoice), invoice.balance_due_cents, invoice.created_at, invoice.due_at, invoice.paid_at])];
  if (view === "profitability") return [["Job ID", "Customer", "Revenue cents", "Approved labor hours", "Labor cost cents", "Approved direct cost cents", "Equipment cost cents", "Estimated gross profit cents", "Estimated margin percent", "Completeness"], ...data.profitability.map((row) => [row.job.id, relation(row.job.customers)?.display_name ?? "Unknown", row.revenueCents, row.laborHours.toFixed(2), row.laborCostCents, row.directCostCents, row.equipmentCostCents, row.profitCents, row.marginPercent?.toFixed(3) ?? null, row.completeness])];
  if (view === "labor") return [["Employee", "Time type", "Job ID", "Clock in", "Clock out", "Break minutes", "Entry status", "Latest review"], ...data.timeEntries.map((entry) => [relation(entry.profiles)?.full_name || relation(entry.profiles)?.email || "Unknown", entry.time_type, entry.job_id, entry.clock_in_at, entry.clock_out_at, entry.break_minutes, entry.status, [...entry.time_entry_approvals].sort((a, b) => b.approved_at.localeCompare(a.approved_at))[0]?.approval_status ?? "unreviewed"])];
