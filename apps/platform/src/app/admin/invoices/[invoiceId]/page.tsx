...
import { ClipboardCheck, FileText, MapPin, ReceiptText, StickyNote, UsersRound } from "lucide-react";
        ...
        {!detail.data ? (
          ...
          <>
            <section className="page-heading">
              <p className="surface-label"><ReceiptText aria-hidden="true" size={18} />Invoice File</p>
              <h1>{detail.data.invoice_number || "Draft invoice"}</h1>
              <p>Invoice workflow is connected to the customer, job, quote, and line items. Payments remain placeholders.</p>
            </section>

            <section className="detail-grid">
              <article className="detail-panel">
                <PanelTitle icon={<ClipboardCheck size={18} />} title="Invoice status" />
                <span className="status-pill">{detail.data.status.replace("_", " ")}</span>
              ...
              <article className="detail-panel">
                <PanelTitle icon={<ReceiptText size={18} />} title="Payment status" />
                <dl className="record-details">
                  ...
                  <div><dt>Due date</dt><dd>{detail.data.due_at ? new Date(detail.data.due_at).toLocaleDateString() : "Not set"}</dd></div>
              ...
              <article className="detail-panel wide-detail-panel">
                ...
                {detail.data.invoice_line_items?.length ? (
                  <div className="line-items-preview">
                    ...
                    <div className="line-item-total"><span>Balance due</span><strong>{formatCurrency(detail.data.balance_due_cents)}</strong></div>
                  </div>
            ...
            <section className="document-workspace">
              <div className="document-workspace-heading print-hidden">
                <div>
                  <p className="surface-label"><ReceiptText aria-hidden="true" size={18} />Printable invoice</p>
