...
- Apply `supabase/migrations/20260720130614_payment_activation_blockers.sql` before any future card activation. It adds dispute accounting and a 30-minute abandoned-authentication deadline; cleanup always checks Stripe and never expires `processing`, `requires_capture`, or `succeeded` PaymentIntents.
- Production already contains the schema from `20260720074140_stripe_card_confirmation_flow.sql`, but its migration history is missing that timestamp. After verifying the schema, the owner can record history without rerunning SQL using `npx supabase migration repair --linked --status applied 20260720074140`, then confirm both columns align with `npx supabase migration list`.
- Production had zero historical contracting-party violations on July 20, 2026. Review and apply `supabase/migrations/20260720130615_validate_contracting_party_constraints.sql` deliberately; it validates existing checks without rewriting business records.
