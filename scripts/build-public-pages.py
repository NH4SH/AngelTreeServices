...
def page_document(page: dict) -> str:
    ...
    if page.get("hero_image"):
        ...
        <figure class="ats-page-hero__media">
          <img src="{page['hero_image']}" width="{page['image_width']}" height="{page['image_height']}" alt="{page['hero_alt']}" decoding="async" fetchpriority="high">
        </figure>"""
    else:
        hero_class = " ats-page-hero__inner--single"

    schema = json.dumps(schema_for_page(page), ensure_ascii=False, separators=(",", ":"))
...
<head>
  ...
  <link rel="stylesheet" href="/site-pages.css?v=batch2">
    ...
    <section class="ats-page-hero">
      <div class="ats-page-hero__inner{hero_class}">
        <div class="ats-page-hero__content">
          {breadcrumbs(page)}
          <h1>{page['h1']}</h1>
          <p class="ats-page-hero__lead">{page['lead']}</p>
          <div class="ats-actions">
            <a class="ats-button ats-button--primary ats-estimate-link" href="{page['estimate_href']}">{page['estimate_label']}</a>
...
{footer()}
  <script src="/site-pages.js" defer></script>
</body>
</html>
"""
...
SERVICE_PAGES = [
    ...
    },
    {
        ...
        "related": [("/services/tree-removal/", "Tree removal planning"), ("/services/emergency-tree-service/", "Storm and emergency guidance"), ("/credentials-safety/", "Credentials and safety")],
    ...
    },
    {
        ...
        "related": [("/services/tree-removal/", "Tree removal planning"), ("/services/tree-pruning/", "Tree pruning"), ("/credentials-safety/", "Credentials and safety")],
...
def build() -> None:
    for page in PAGES:
        destination = ROOT / page["path"].strip("/") / "index.html"
